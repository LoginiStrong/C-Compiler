...
ASTBooleanExprB* Parser::rdpBooleanExprB()
{
	...
	while ((scan->currentTokenType() == EE) || (scan->currentTokenType() == GR) || (scan->currentTokenType() == GRE) || (scan->currentTokenType() == LS))
	{
		node->addOperator(scan->currentTokenString());
...
{
	ASTFor* forNode = new ASTFor();
	forNode->setLine(scan->getLine());
	...
	{
		printError("rdpFor02", "Expected (", scan->getLine());

		scan->advance(); // always advance in an error, otherwise you might get stuck in a loop
	}

	forNode->setAssign1(rdpAssign());
	
	forNode->setBoolExprA(rdpBooleanExprA());
	if (scan->currentTokenType() == SEMI)
	{
		scan->advance(); // advance the scanner to look at the next thing
	}
	...
	{
		printError("rdpFor03", "Expected ;", scan->getLine());

		scan->advance(); // always advance in an error, otherwise you might get stuck in a loop
	}

	forNode->setAssign2(rdpAssign());
	...
	{
		printError("rdpFor04", "Expected )", scan->getLine());

		scan->advance(); // always advance in an error, otherwise you might get stuck in a loop
	}

	forNode->setStatement(rdpStatement());
	return forNode;
}