...
Register ASTWhile::output(VariableContext* vc, ofstream& out)
{
	...
	out << "label l" << l1 << "\n";
	//check condition
	Register condr = condition->output(vc, out);
	//not condition
	out << "! " << condr.toString() << " " << condr.toString() << "\n";
	//jump if condition
	out << "jumpif l" << l2 << " " << condr.toString() << "\n";

	//output body
	VariableContext* tm = new VariableContext();

	for (auto it = vc->begin(); it != vc->end(); it++)
	{
		tm->operator[](it->first) = it->second;
	}
	statement->output(tm, out);
...
{
	Register r2 = rhs->output(vc, out);
	Register r = (*vc)[this->lhs];
	out << "= " << r.toString() << " " << r2.toString() << "\n";
	return NOREG;
}
...
//like before, I did a bit of these to make part 1 work.
Register  ASTBooleanExprA::output(VariableContext* vc, ofstream& out)
{
	if (operands.size() == 0)
		return operands[0]->output(vc, out);

	Register lastResult = operands[0]->output(vc, out);
	for (int i = 0; i < operands.size() - 1; i++)
	{
		Register newResult = regs.getBoolReg();
		out << "bool " << newResult.toString() << "\n";

		Register r = operands[i + 1]->output(vc, out);

		if (operators[i] == "|")
		{
			out << "|| " << newResult.toString() << " " << lastResult.toString() << " " << r.toString() << "\n";
		}
		else
		{
			out << "&& " << newResult.toString() << " " << lastResult.toString() << " " << r.toString() << "\n";
...
Register  ASTBooleanExprB::output(VariableContext* vc, ofstream& out)
{
	...
	for (int i = 0; i < operands.size() - 1; i++)
	{
		...
		{
			Register r = operands[i + 1]->output(vc, out);
			out << "> " << newResult .toString() << " " << r.toString() << " " << lastResult.toString() << "\n";
		}
		else
		{
			Register r = operands[i + 1]->output(vc, out);
...
Register  ASTTerm::output(VariableContext* vc, ofstream& out)
{
	...
	for (int i = 0; i < operands.size() - 1; i++)
	{
		Register newResult = regs.getFloatReg();
		out << "float " << newResult.toString() << "\n";
...
void ASTFor::setAssign1(ASTNode* statementin)
{
	assign1 = statementin;
}

void ASTFor::setBoolExprA(ASTNode* statementin)
{
	boolExprA = statementin;
}

void ASTFor::setStatement(ASTNode* statementin)
{
	statement = statementin;
}

void ASTFor::setAssign2(ASTNode* statementin)
{
	assign2 = statementin;
}
...
{
	hadError = false;
	assign1->typeCheck(map, fmap, hadError);
	boolExprA->typeCheck(map, fmap, hadError);
	assign2->typeCheck(map, fmap, hadError);
...
Register ASTFor::output(VariableContext* vc, ofstream& out)
{
	int l1 = labelcount++;
	int l2 = labelcount++;

	Register first = assign1->output(vc, out);
	Register sec;

	out << "label l" << l1 << "\n";
	Register condition = boolExprA->output(vc, out);
	out << "! " << condition.toString() << " " << condition.toString() << "\n";

	out << "jumpif l" << l2 << " " << condition.toString() << "\n";
